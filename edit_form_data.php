...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   if (isset($_POST['id']) && !empty($_POST['id']) && trim($_POST['id'])) {
    ...
    if (count($err) === 0) {
        ...
        //insert data  sql
        $sql = "update tbl_products set name='$name', price=$price where id=$id";
            //execute query
...
<div class="container">
    ...
    <a href="list_table.php">List Product</a>
        ...
        <?php } ?>
    <form action="<?php echo $_SERVER['PHP_SELF'] ?>?pid=<?php echo $id ?>" method="POST">
        <label for="id">Product ID</label>
        <input type="text" id="id" name="id" value="<?php echo $id??'' ?>" placeholder="Enter Product ID" readonly>
